...
}
{
  ...
  function HexFloat() {
    document.hexfloat.hf.value +=
      document.hexfloat.hex.value +
      " = " +
      document.hexfloat.float.value +
      "\n";
  }

  function HexDec() {
    document.hexdec.hd.value +=
      document.hexdec.hex.value + " = " + document.hexdec.dec.value + "\n";
  }

  function SaveASCII() {
    document.asciihex.ah.value +=
  ...
  function ToASCII() {
    ...
    if (document.getElementById("UTF8").checked)
      ...
      document.asciihex.ascii.value = "";
      while (HexString != "") {
        document.asciihex.ascii.value += String.fromCharCode(
          parseInt(HexString.substr(0, 2), 16) & 255
        );
  ...
  function FromASCII() {
    ...
    if (document.getElementById("UTF8").checked) {
      ...
      for (var i = 0; i < input.length; i += 2) {
        if (i % 8 == 0 && i > 0)
          i % 16 == 0 ? (codeformatted += lnbreaks) : (codeformatted += " ");
      ...
      for (var x = 0; ASCIIString != ""; x++) {
        if (x % 4 == 0 && x > 0)
          x % 8 == 0
            ? (document.asciihex.hex.value += lnbreaks)
            : (document.asciihex.hex.value += " ");
        if (Number(ASCIIString.charCodeAt(0)) < 16)
          document.asciihex.hex.value += "0";
        document.asciihex.hex.value += Number(ASCIIString.charCodeAt(0))
          .toString(16)
          .toUpperCase();
  ...
  function DecimalToHex() {
    if (Number(document.hexdec.dec.value) < Number(-2147483648)) {
      ...
      document.hexdec.hex.value = (
        Number(document.hexdec.dec.value) +
        0xffffffff +
        1
      )
        .toString(16)
      ...
      document.hexdec.hex.value = Number(document.hexdec.dec.value)
        .toString(16)
        .toUpperCase()
        .substr(0, 8);
      while (document.hexdec.hex.value.length < 8)
        document.hexdec.hex.value = "0" + document.hexdec.hex.value;
    }
  }

  function HexToDecimal() {
    if (
      document.hexdec.Signed.checked &&
      parseInt(document.hexdec.hex.value, 16) > 2147483647
    )
      document.hexdec.dec.value = -(
        ~parseInt(document.hexdec.hex.value, 16) + 1
      );
    else document.hexdec.dec.value = parseInt(document.hexdec.hex.value, 16);
  }
  ...
  function Buttons(Bob) {
    if (!AddressOkay) return;
    Bob.style.backgroundColor =
      "#" +
      (Bob.title != "Included - " + Bob.alt
        ? document.stuff.ColorClick.value
        : document.stuff.ColorNormal.value);
    Bob.title != "Included - " + Bob.alt
      ? (Bob.title = "Included - " + Bob.alt)
      : (Bob.title = Bob.alt);
    Bob.form.OR.value = Number(
      parseInt(Bob.value, 16) ^ parseInt(Bob.form.OR.value, 16)
    )
      .toString(16)
      .toUpperCase();
    while (Bob.form.OR.value.length < 4)
      Bob.form.OR.value = "0" + Bob.form.OR.value;
    Bob.form.NOT.value = Number(~parseInt(Bob.form.OR.value, 16) + 0xffff + 1)
      .toString(16)
      .toUpperCase();
  ...
  function OverButton(Bob, Enter) {
    ...
    if (Bob.title != "Included - " + Bob.alt)
      Bob.style.backgroundColor =
        "#" +
        (Enter
          ? document.stuff.ColorHover.value
          : document.stuff.ColorNormal.value);
  ...
  function Validate() {
    ...
    document.results.Address.style.backgroundColor = AddressOkay
      ? "transparent"
      : "#FFCCCC";
    ValidateSetButtons();
    if (!AddressOkay) return;
    document.results.Address.value = Number(
      0xffffffff +
        (0xf1ffffff &
          ((parseInt(document.results.Address.value, 16) >>> 1) << 1)) +
        1
    )
      .toString(16)
      .toUpperCase();
  ...
  function Update() {
    ...
    var FullAddress =
      (((parseInt(document.results.Address.value, 16) << 7) >>> 8) << 1) +
      (document.results.Endif.checked ? 1 : 0);
    ResultString +=
      Number((parseInt(document.results.CT.value, 16) << 24) + FullAddress)
        .toString(16)
        .toUpperCase() + " ";
    var WhereFrom = document.getElementsByName(document.results.Current.value);

    while (WhereFrom[0].NOT.value.length < 4)
      WhereFrom[0].NOT.value = "0" + WhereFrom[0].NOT.value;

    if (document.results.Inclusive.checked)
      ResultString += WhereFrom[0].NOT.value;
    else ResultString += "0000";

    ResultString += WhereFrom[0].OR.value;

    var Bytes = new Array(
      ResultString.substr(9, 2),
      ResultString.substr(11, 2),
      ResultString.substr(13, 2),
      ResultString.substr(15, 2)
    );

    if (document.results.Reverse.checked)
      ResultString =
        ResultString.substr(0, 9) + Bytes[1] + Bytes[0] + Bytes[3] + Bytes[2];
    if (WhereFrom[0].ORle) WhereFrom[0].ORle.value = Bytes[3] + Bytes[2];
    if (WhereFrom[0].NOTle)
      WhereFrom[0].NOTle.value =
        WhereFrom[0].NOT.value.substr(2, 2) +
        WhereFrom[0].NOT.value.substr(0, 2);
  ...
  function ResetData() {
    ...
    if (WhereFrom.length != 0) {
      ...
      var FormButton = document.getElementsByName(
        "bttn" + document.results.Current.value
      );
      for (var x = 0; x < FormButton.length; x++) {
        if (FormButton[x].form.name == document.results.Current.value) {
          ...
          FormButton[x].style.backgroundColor =
            "#" + document.stuff.ColorNormal.value;
        }
  ...
  function ValidateSetButtons() {
    SetButtonsOkay =
      /^[A-Fa-f0-9]{4}$/.test(document.results.ToButtons.value) && AddressOkay;
    if (!SetButtonsOkay) {
    ...
    document.results.ToButtons.value =
      document.results.ToButtons.value.toUpperCase();
  }

  function SetButtons() {
    if (!SetButtonsOkay || !AddressOkay) return;
  ...
  function DebugDump(ToThisID, StartAt) {
    ...
    var TableSize = 20;
    FullTable =
      "<table border=0 width='100%' style='text-align: center'>" + TableRow;
    for (var x = SA; x < AllInputs.length && x < TableSize + SA; x++) {
      ...
      AddThis = AddThis.replace(
        "Input Value",
        AllInputs[x].value.substr(0, 12)
      );
      FullTable += AddThis;
    }
    FullTable += "</table><br>&nbsp;Debugger - Dump At Page:";
    for (var x = 0; x + TableSize / 2 < AllInputs.length; x += TableSize / 2) {
      if (x % 12 == 0) FullTable += "<br>";
      FullTable +=
        "&nbsp;&nbsp;<a href='#' style=\"background: #000000; color: #FFFFFF; font-family: monospace;\" onclick=\"DebugDump('" +
        ToThisID +
        "', '" +
        x +
        "'); return false;\">" +
        Number(x / 20).toString(10) +
        "</a>";
    }
  ...
  function RandomColor() {
    ...
    RRGGBB +=
      RRGGBB == "FFFF" ? "00" : RRGGBB == "0000" ? "FF" : ms & 4 ? "FF" : "00";
    return RRGGBB;
  }
}
