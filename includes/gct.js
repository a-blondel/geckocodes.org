...
}
{
  ...
  var Code =
    "<a href='#editbox' onclick=\"Edit('c[count]'); return false\">Edit</a> <input type='checkbox' name='t[count]' value='0' onClick=\"Order(this);\"> [codetitle]\n<input type='hidden' name='T[count]' value='[codetitle]'><textarea name='c[count]' rows=5 cols=40 class='hide'>[codebody]</textarea>[hasnote]<div class=note id='n[count]'>[note]</div><br>";

  function IsCode(Bob) {
    //bob is the code string
    var Code = trim(Bob.replace(/\r/gi, ""));
    var Lines = Code.split("\n");
    for (var i = 0; i < Lines.length; i++) {
      Lines[i] = trim(Lines[i]);
      if (!(Lines[i].length == 17 && Lines[i].charAt(8) == " ")) return 0;
    }
    Code = Code.replace(/\n/gi, "").replace(/ /gi, "");
    if (Code.length % 8 != 0) return 0;
    for (var i = 0; i < Code.length; i++) {
      if (!Code.charAt(i).match(/^[A-F0-9]$/)) return 0;
    }
  ...
  function Order(Bob) {
    if (Bob.checked) {
      ...
      document.gct.gctorder.value = document.gct.gctorder.value.replace(
        Bob.name + " ",
        ""
      );
  }
  ...
  function AddCodes() {
    var byline;
    var endline = "\n";
    byline = document.gct.newcodes.value
      .replace(/\r/gi, "")
      .replace(/</gi, "")
      .replace(/>/gi, "")
      .split(endline);
    ...
    for (var i = 0; i < byline.length; i++) {
      ...
      if (byline[i] != "") {
        ...
        for (
          ...
          i < byline.length &&
          byline[i] != "" &&
          trim(byline[i]).length == 17 &&
          trim(byline[i]).charAt(8) == " ";
          i++
        ) {
          ToWrite = ToWrite.replace(
            "[codebody]",
            trim(byline[i]) + endline + "[codebody]"
          );
        }
    ...
    for (var i = 0; i < byline.length; i++) {
      if (document.gct[byline[i]]) document.gct[byline[i]].checked = true;
    }
  ...
  function Edit(Bob) {
    if (document.gct[Bob]) {
      ...
      document.gct.editareaTitle.value =
        document.gct[Bob.replace("c", "T")].value;
      document.gct.editareaNote.value = document
        .getElementById(Bob.replace("c", "n"))
  ...
  function ApplyEdit() {
    if (
      document.gct[document.gct.editing.value] &&
      IsCode(document.gct.editarea.value) != 0
    ) {
      document.gct[document.gct.editing.value].value =
        document.gct.editarea.value;
      document.gct[document.gct.editing.value.replace("c", "t")].value = IsCode(
        document.gct.editarea.value
      );
  ...
  function Toggle(Bob) {
    ...
    else document.getElementById(Bob).style.display = "none";
  }
  ...
  function CreateGCT() {
    ...
    for (var i = 0; i < Bob.length; i++) {
      if (document.gct[Bob[i]])
        document.fin.gctdata.value += document.gct[Bob[i]].value
          .replace(/\r/gi, "")
          .replace(/\n/gi, "")
          .replace(/ /gi, "");
    }
    if (document.fin.gctdata.value == "00D0C0DE00D0C0DE") {
      ...
      var bytes = new Uint8Array(
        document.fin.gctdata.value.match(/[\da-f]{2}/gi).map(function (h) {
